...
abstract class BaseJenisSertifikasi extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'DataDikdas\\Model\\JenisSertifikasiPeer';
    ...
    /**
     * Initializes internal state of BaseJenisSertifikasi object.
     * @see        applyDefaults()
     */
    ...
    /**
     * Set the value of [a_pd] column.
     * 
     * @param string $v new value
     * @return JenisSertifikasi The current object (for fluent API support)
     */
    public function setAPd($v)
    {
        if ($v !== null && is_numeric($v)) {
            $v = (string) $v;
        }
        ...
        if ($this->a_pd !== $v) {
            $this->a_pd = $v;
            $this->modifiedColumns[] = JenisSertifikasiPeer::A_PD;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->a_pd = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
            $this->kebutuhan_khusus_id = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
            ...
            return $startcol + 11; // 11 = JenisSertifikasiPeer::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(JenisSertifikasiPeer::ID_JENIS_SERTIFIKASI)) {
            $modifiedColumns[':p' . $index++]  = '"id_jenis_sertifikasi"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::JENIS_SERTIFIKASI)) {
            $modifiedColumns[':p' . $index++]  = '"jenis_sertifikasi"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::PROF_GURU)) {
            $modifiedColumns[':p' . $index++]  = '"prof_guru"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::KEPALA_SEKOLAH)) {
            $modifiedColumns[':p' . $index++]  = '"kepala_sekolah"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::LABORAN)) {
            $modifiedColumns[':p' . $index++]  = '"laboran"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::A_PD)) {
            $modifiedColumns[':p' . $index++]  = '"a_pd"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::KEBUTUHAN_KHUSUS_ID)) {
            $modifiedColumns[':p' . $index++]  = '"kebutuhan_khusus_id"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::CREATE_DATE)) {
            $modifiedColumns[':p' . $index++]  = '"create_date"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::LAST_UPDATE)) {
            $modifiedColumns[':p' . $index++]  = '"last_update"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::EXPIRED_DATE)) {
            $modifiedColumns[':p' . $index++]  = '"expired_date"';
        }
        if ($this->isColumnModified(JenisSertifikasiPeer::LAST_SYNC)) {
            $modifiedColumns[':p' . $index++]  = '"last_sync"';
        }

        $sql = sprintf(
            'INSERT INTO "ref"."jenis_sertifikasi" (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '"jenis_sertifikasi"':						
                        $stmt->bindValue($identifier, $this->jenis_sertifikasi, PDO::PARAM_STR);
                        break;
                    case '"prof_guru"':						
                        $stmt->bindValue($identifier, $this->prof_guru, PDO::PARAM_STR);
                        break;
                    case '"kepala_sekolah"':						
                        $stmt->bindValue($identifier, $this->kepala_sekolah, PDO::PARAM_STR);
                        break;
                    case '"laboran"':						
                        $stmt->bindValue($identifier, $this->laboran, PDO::PARAM_STR);
                        break;
                    case '"a_pd"':						
                        $stmt->bindValue($identifier, $this->a_pd, PDO::PARAM_STR);
                        break;
    ...
    public function toArray($keyType = BasePeer::TYPE_FIELDNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[5] => $this->getAPd(),
            $keys[6] => $this->getKebutuhanKhususId(),
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_FIELDNAME)
    {
        ...
        if (array_key_exists($keys[5], $arr)) $this->setAPd($arr[$keys[5]]);
        if (array_key_exists($keys[6], $arr)) $this->setKebutuhanKhususId($arr[$keys[6]]);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(JenisSertifikasiPeer::A_PD)) $criteria->add(JenisSertifikasiPeer::A_PD, $this->a_pd);
    ...
    public function setRwySertifikasis(PropelCollection $rwySertifikasis, PropelPDO $con = null)
    {
        ...
        foreach ($rwySertifikasisToDelete as $rwySertifikasiRemoved) {
            $rwySertifikasiRemoved->setJenisSertifikasi(null);
        }
    ...
    protected function doAddRwySertifikasi($rwySertifikasi)
    {
        ...
        $rwySertifikasi->setJenisSertifikasi($this);
    }
    ...
    public function removeRwySertifikasi($rwySertifikasi)
    {
        if ($this->getRwySertifikasis()->contains($rwySertifikasi)) {
            ...
            $rwySertifikasi->setJenisSertifikasi(null);
        }
    ...
    public function setSertifikasiPds(PropelCollection $sertifikasiPds, PropelPDO $con = null)
    {
        ...
        foreach ($sertifikasiPdsToDelete as $sertifikasiPdRemoved) {
            $sertifikasiPdRemoved->setJenisSertifikasi(null);
        }
    ...
    protected function doAddSertifikasiPd($sertifikasiPd)
    {
        ...
        $sertifikasiPd->setJenisSertifikasi($this);
    }
    ...
    public function removeSertifikasiPd($sertifikasiPd)
    {
        if ($this->getSertifikasiPds()->contains($sertifikasiPd)) {
            ...
            $sertifikasiPd->setJenisSertifikasi(null);
        }
